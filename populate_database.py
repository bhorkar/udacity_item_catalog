...
# Create fake usersa
    ...
    users = [{'username': 'test', 'email': 'test@gmail.com',
    ...
    def add_users(self):
        ...
        self.session.commit()
        user_genesis = self.session.query(
            User).filter_by(email='test@gmail.com').one()
        cate1 = Category(name='Soccer', user_id=user_genesis.id)
        self.session.add(cate1)
        self.session.commit()
        item1 = CatalogItem(
            name="FIFA shirt women",
            description="FIFA marks on the back and a big pink ball on the front",
            user_id=user_genesis.id,
            category=cate1)
        self.session.add(item1)
        self.session.commit()
        item2 = CatalogItem(
            name="FIFA shirt men",
            description="FIFA marks on the back and a big black heart on the front",
            user_id=user_genesis.id,
            category=cate1)
        ...
        self.session.commit()
        item1 = CatalogItem(
            name="Nike Leather Basketballs",
            description="Made out of fake leather",
            user_id=user_genesis.id,
            category=cate2)
        self.session.add(item1)
        self.session.commit()
        item2 = CatalogItem(
            name="Oversized Basketball vests",
            description="rainball colors with different sizes",
            user_id=user_genesis.id,
            category=cate2)
        self.session.add(item2)
        self.session.commit()
        ...
        item1 = CatalogItem(
            name="Yankee hats",
            description="all hats are designer hats and the materials are extremely recyclable",
            user_id=user_genesis.id,
            category=cate3)
        self.session.add(item1)
        self.session.commit()
        item2 = CatalogItem(
            name="baseballs bats",
            description="Different colors are available. You can also pre-order with your chosen color",
            user_id=user_genesis.id,
            category=cate3)
        self.session.add(item2)
        self.session.commit()
        ...
        item1 = CatalogItem(
            name="Goggles",
            description="Goggles are made out of lime stones, and they all are imported from Netherland",
            user_id=user_genesis.id,
            category=cate4)
        self.session.add(item1)
        self.session.commit()
        ...
        item2 = CatalogItem(
            name="Snowboards",
            description="Different Length and different designs are available for all professional levels",
            user_id=user_genesis.id,
            category=cate4)
        self.session.add(item2)
        self.session.commit()
        ...
        item1 = CatalogItem(
            name="Swimming suits",
            description="Roxy brands and alike are available. \
        ...
        item2 = CatalogItem(
            ...
            description="They are all on sale. Different colors are available",
            user_id=user_genesis.id,
            category=cate5)
        self.session.add(item2)
        self.session.commit()
        ...
        for cate in cates:
            ...
            items = self.session.query(
                CatalogItem).filter_by(category=cate).all()
            for item in items:
                print(item.name)
